...
class OrphanPortalListViewModel(
    private val portalSource: DataSource<Portal>,
    private val transformer: ViewModelListTransformer<List<Portal>>
) : SimpleViewModel<OrphanPortalListViewModel, OrphanPortalListFragment>(), OrphanPortalContract.ViewModel {


    override suspend fun getData(): List<Item.ViewModel> {
        return withContext(Dispatchers.IO) {
            transformer.transform(portalSource.getAllOrphans())
        }
    }
